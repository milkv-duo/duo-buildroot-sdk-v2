...
void axi_mon_get_info_all(uint32_t duration)
{
	...
	if (dram_info.data_rate) {
		...
		for (i = 0; i <= 10; i++) {
			...
			//aximon_info.m1.write_latency_his_cnt[i], aximon_info.m1.read_latency_his_cnt[i]);
		}
...
void axi_mon_dump_single(struct AXIMON_INFO_PORT *port_info)
{
	...
	if (strcmp(port_info->port_name, "total") != 0) {
		...
		pr_err("avg_read_latency=%5dns, avg_read_latency_min=%5dns, avg_read_latency_max=%5dns\n",
