...
void Wave4BitIssueCommand(CodecInst *instance, Uint32 cmd)
{
	...
	if (instance->productId == PRODUCT_ID_7Q) // only coda7q use
		// codecModeAux for DIV
		VpuWriteReg(coreIdx, W4_INST_INDEX,
			    (instanceIndex & 0xffff) | (codecMode << 16) |
				    (instance->codecModeAux << 24));
	else
#endif
		VpuWriteReg(coreIdx, W4_INST_INDEX,
			    (instanceIndex & 0xffff) | (codecMode << 16));
...
static RetCode SetupWave4Properties(Uint32 coreIdx)
{
...
#ifdef REDUNDENT_CODE
	case WAVE410_CODE:
		pAttr->productId = PRODUCT_ID_410;
		break;
	case WAVE4102_CODE:
		pAttr->productId = PRODUCT_ID_4102;
		break;
	case WAVE420_CODE:
		pAttr->productId = PRODUCT_ID_420;
		break;
	case WAVE412_CODE:
		pAttr->productId = PRODUCT_ID_412;
		break;
	case CODA7Q_CODE:
...
#ifdef REDUNDENT_CODE
	case WAVE510_CODE:
		pAttr->productId = PRODUCT_ID_510;
		break;
	case WAVE512_CODE:
		pAttr->productId = PRODUCT_ID_412;
		break;
	case WAVE515_CODE:
		pAttr->productId = PRODUCT_ID_515;
		break;
#endif
	default:
	...
	if (pAttr->productId == PRODUCT_ID_420L) {
		...
		pAttr->supportDecoders =
			...
			(1 << STD_DIV3) | (1 << STD_RV) | (1 << STD_THO) |
			(1 << STD_VP8) | (1 << STD_HEVC);
...
#endif
	pAttr->supportFBCBWOptimization =
		(BOOL)((pAttr->hwConfigDef1 >> 15) & 0x01);
	...
	pAttr->supportEndianMask =
		(Uint32)((1 << VDI_LITTLE_ENDIAN) | (1 << VDI_BIG_ENDIAN) |
			 (1 << VDI_32BIT_LITTLE_ENDIAN) |
			 (1 << VDI_32BIT_BIG_ENDIAN) | (0xffff << 16));
	pAttr->supportBitstreamMode =
		(1 << BS_MODE_INTERRUPT) | (1 << BS_MODE_PIC_END);
	pAttr->framebufferCacheType = FramebufCacheNone;
	pAttr->bitstreamBufferMargin =
		(pAttr->productId == PRODUCT_ID_7Q) ? 1024 : 0;
...
#ifdef REDUNDENT_CODE
	case WAVE410_CODE:
		productId = PRODUCT_ID_410;
		break;
	case WAVE4102_CODE:
		productId = PRODUCT_ID_4102;
		break;
	case WAVE420_CODE:
		productId = PRODUCT_ID_420;
		break;
	case WAVE412_CODE:
		productId = PRODUCT_ID_412;
		break;
	case CODA7Q_CODE:
...
RetCode Wave4VpuInit(Uint32 coreIdx, void *firmware, Uint32 size)
{
	...
	VpuWriteMem(coreIdx, codeBase, (unsigned char *)firmware, size * 2,
	...
	CVI_VC_PERF("Wave4 LoadFirmware = %llu us\n", end_time - start_time);

	/* remap page size */
	remapSize = (codeSize >> 12) & 0x1ff;
	regVal = 0x80000000 | (0 << 16) | (W4_REMAP_CODE_INDEX << 12) |
		 (1 << 11) | remapSize;
	...
	if (regVal == 0) {
		...
		CVI_VC_ERR(
			"VPU init(W4_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n",
			regVal, reasonCode);
		return RETCODE_FAILURE;
	}
...
RetCode Wave4VpuReInit(Uint32 coreIdx, void *firmware, Uint32 size)
{
	...
	oldCodeBase = vdi_remap_memory_address(
		coreIdx, VpuReadReg(coreIdx, W4_VPU_REMAP_PADDR));

	if (oldCodeBase != codeBase) {
		VpuWriteMem(coreIdx, codeBase, (unsigned char *)firmware,
			    size * 2, VDI_128BIT_LITTLE_ENDIAN);
		vdi_set_bit_firmware_to_pm(coreIdx, (Uint16 *)firmware);
		...
		vdi_fio_write_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL,
				       0x100); // WAVE410 GDI added new feature:
		// disable_request

		// Step2 : Waiting for completion of bus transaction
		if (vdi_wait_bus_busy(coreIdx, __VPU_BUSY_TIMEOUT,
				      W4_GDI_VCORE0_BUS_STATUS) == -1) {
			vdi_fio_write_register(coreIdx, W4_GDI_VCORE0_BUS_CTRL,
					       0x00);
#ifdef ENABLE_CNM_DEBUG_MSG
		...
		/* Reset All blocks */
		regVal = 0x7ffffff;
		VpuWriteReg(coreIdx, W4_VPU_RESET_REQ,
...
RetCode Wave4VpuSleepWake_EX(Uint32 coreIdx, int iSleepWake, Uint32 size)
{
	...
	if (iSleepWake == 1) {
		...
		regVal = VpuReadReg(coreIdx, W4_RET_SUCCESS);
		if (regVal == 0) {
			APIDPRINT("SLEEP_VPU failed [0x%x]",
		...
		regVal = W4_RST_BLOCK_ACLK_ALL | W4_RST_BLOCK_BCLK_ALL |
			 W4_RST_BLOCK_CCLK_ALL;
		VpuWriteReg(coreIdx, W4_VPU_RESET_REQ,
			    regVal); // Reset All blocks
		...
		if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT,
				      W4_VPU_BUSY_STATUS) == -1) {
			VLOG(ERR,
			     "VPU Wakeup (W4_VPU_REMAP_CORE_START) timeout\n");
			CVI_VC_ERR("RESPONSE_TIMEOUT\n");
			return RETCODE_VPU_RESPONSE_TIMEOUT;
		}
		...
		if (regVal == 0) {
			Uint32 reasonCode =
				VpuReadReg(coreIdx, W4_RET_FAIL_REASON);
			VLOG(ERR,
			     "VPU Wakeup(W4_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n",
			     regVal, reasonCode);
...
#ifdef REDUNDENT_CODE
		if (hdr.productId == PRODUCT_ID_7Q) // only coda7q use
			// codecModeAux for DIV
			VpuWriteReg(coreIdx, W4_INST_INDEX,
				    (hdr.instIndex & 0xffff) |
					    (hdr.codecMode << 16) |
					    (hdr.codecModeAux << 24));
		else
#endif
...
RetCode Wave4VpuReset(Uint32 coreIdx, SWResetMode resetMode)
{
	...
	if (pAttr->supportGDIHW == TRUE) {
		...
		if (pAttr->productId == PRODUCT_ID_420L) {
			val = vdi_fio_read_register(coreIdx,
						    W4_GDI_VCORE0_BUS_CTRL);
			if ((val >> 24) == 0x01) {
				/* VPU has a bus transaction controller */
				vdi_fio_write_register(
					coreIdx, W4_GDI_VCORE0_BUS_CTRL, 0x11);
			}
			if (vdi_wait_bus_busy(coreIdx, __VPU_BUSY_TIMEOUT,
					      W4_GDI_VCORE0_BUS_STATUS) == -1) {
#ifdef ENABLE_CNM_DEBUG_MSG
	...
	case SW_RESET_SAFETY:
		val = W4_RST_BLOCK_ACLK_ALL | W4_RST_BLOCK_BCLK_ALL |
		      W4_RST_BLOCK_CCLK_ALL;
		break;
	default:
	...
	if (pAttr->supportGDIHW == TRUE) {
		...
		if (pAttr->productId == PRODUCT_ID_420L) {
			...
			if ((val >> 24) == 0x01) {
				/* VPU has a bus transaction controller */
				vdi_fio_write_register(
					coreIdx, W4_GDI_VCORE0_BUS_CTRL, 0);
			}
		}
	}
...
RetCode Wave4VpuBuildUpDecParam(CodecInst *instance, DecOpenParam *param)
{
	...
	if (param->vbWork.size > 0) {
...
#ifdef REDUNDENT_CODE
		if ((instance->productId == PRODUCT_ID_412) ||
		    (instance->productId == PRODUCT_ID_512) ||
		    (instance->productId == PRODUCT_ID_515)) {
		...
		sprintf(ionName, "VDEC_%d_H265_WorkBuffer",
			instance->s32ChnNum);
		if (VDI_ALLOCATE_MEMORY(instance->coreIdx, &pDecInfo->vbWork, 0,
					ionName) < 0) {
	...
	vdi_clear_memory(instance->coreIdx, pDecInfo->vbWork.phys_addr,
			 pDecInfo->vbWork.size);

	VpuWriteReg(instance->coreIdx, W4_ADDR_WORK_BASE,
...
RetCode Wave4VpuDecSetBitstreamFlag(CodecInst *instance, BOOL running, BOOL eos,
				    BOOL explicitEnd)
{
	...
	if (running == TRUE) {
		BitStreamMode bsMode =
			(BitStreamMode)pDecInfo->openParam.bitstreamMode;
		BOOL explicitEndFlag = 0;

		explicitEndFlag = (BOOL)(bsMode == BS_MODE_PIC_END ||
					 pDecInfo->streamEndflag == TRUE ||
					 explicitEnd == TRUE);
		VpuWriteReg(instance->coreIdx, W4_BS_OPTION,
			    (pDecInfo->streamEndflag << 1) | explicitEndFlag);
	}

	return RETCODE_SUCCESS;
}

RetCode Wave4VpuDecRegisterFramebuffer(CodecInst *inst, FrameBuffer *fbArr,
				       TiledMapType mapType, Uint32 count)
{
	...
	if (mapType == COMPRESSED_FRAME_MAP) {
		...
		if (inst->codecMode == C7_VP9_DEC) {
			mvColSize = WAVE4_DEC_VP9_MVCOL_BUF_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight);
		} else if (inst->codecMode == HEVC_DEC) {
			mvColSize = WAVE4_DEC_HEVC_MVCOL_BUF_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight);
		} else if (inst->codecMode == C7_AVS2_DEC) {
			mvColSize = CODA7Q_MVCOL_BUF_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight); // [FIX ME!!!]
		} else {
			mvColSize = CODA7Q_MVCOL_BUF_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight);
		...
		if (inst->codecMode == HEVC_DEC ||
		    inst->codecMode == C7_HEVC_DEC ||
		    inst->codecMode == C7_VP9_DEC ||
		    inst->codecMode == C7_AVS2_DEC) {
			vbBuffer.size = ((mvColSize + 4095) & ~4095) +
			...
			for (k = 0; k < mvCount; k++) {
				if (pDecInfo->vbMV[k].size == 0) {
					CVI_VC_MEM("vbMV[%d].size = 0x%X\n", k,
						   vbBuffer.size);
					sprintf(ionName,
						"VDEC_%d_H265_mvColBuf",
						inst->s32ChnNum);
					if (VDI_ALLOCATE_MEMORY(inst->coreIdx,
								&vbBuffer, 0,
								ionName) < 0)
						return RETCODE_INSUFFICIENT_RESOURCE;
		...
		if (inst->codecMode == C7_VP9_DEC)
			...
			fbcYTblSize = CODA7Q_FBC_LUMA_TABLE_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight); // [FIX ME!!!]
		else
			fbcYTblSize = CODA7Q_FBC_LUMA_TABLE_SIZE(
		...
		for (k = 0; k < count; k++) {
			if (pDecInfo->vbFbcYTbl[k].size == 0) {
				...
				sprintf(ionName, "VDEC_%d_H265_vbFbcYTbl",
					inst->s32ChnNum);
				if (VDI_ALLOCATE_MEMORY(inst->coreIdx,
							&vbBuffer, 0,
							ionName) < 0)
...
#ifdef REDUNDENT_CODE
		if (inst->codecMode == C7_VP9_DEC)
			fbcCTblSize = WAVE4_FBC_CHROMA_TABLE_SIZE(
				VPU_ALIGN64(pDecInfo->initialInfo.picWidth),
				VPU_ALIGN64(pDecInfo->initialInfo.picHeight));
		else if (inst->codecMode == HEVC_DEC)
			fbcCTblSize = WAVE4_FBC_CHROMA_TABLE_SIZE(
			...
			fbcCTblSize = CODA7Q_FBC_CHROMA_TABLE_SIZE(
				pDecInfo->initialInfo.picWidth,
				pDecInfo->initialInfo.picHeight); // [FIX ME!!!]
		else
			fbcCTblSize = CODA7Q_FBC_CHROMA_TABLE_SIZE(
		...
		for (k = 0; k < count; k++) {
			if (pDecInfo->vbFbcCTbl[k].size == 0) {
				CVI_VC_MEM("vbFbcCTbl[%d].size = 0x%X\n", k,
					   vbBuffer.size);
				memset(ionName, 0, MAX_VPU_ION_BUFFER_NAME);
				sprintf(ionName, "VDEC_%d_H265_vbFbcCTbl",
					inst->s32ChnNum);
				if (VDI_ALLOCATE_MEMORY(inst->coreIdx,
							&vbBuffer, 0,
							ionName) < 0)
		...
		if (inst->productId == PRODUCT_ID_420 ||
		    inst->productId == PRODUCT_ID_420L) {
			lumaStride =
				VPU_ALIGN16(pDecInfo->initialInfo.picWidth) *
				(pDecInfo->initialInfo.lumaBitdepth > 8 ? 5 :
										4);
			lumaStride = VPU_ALIGN32(lumaStride);
			chromaStride =
				VPU_ALIGN16(pDecInfo->initialInfo.picWidth /
					    2) *
				(pDecInfo->initialInfo.chromaBitdepth > 8 ? 5 :
										  4);
			chromaStride = VPU_ALIGN32(chromaStride);
			VpuWriteReg(coreIdx, W4_FBC_STRIDE,
				    lumaStride << 16 | chromaStride);
		}
	}
	endian = vdi_convert_endian(coreIdx, fbArr[0].endian) &
		 VDI_128BIT_ENDIAN_MASK;

	regVal = (pDecInfo->initialInfo.picWidth << 16) |
		 (pDecInfo->initialInfo.picHeight);
	...
	regVal = ((mapType == LINEAR_FRAME_MAP) << 28) | (axiID << 24) |
		 (1 << 23) | /* PIXEL ORDER in 128bit. first pixel in low
				address */
		 (yuvFormat << 20) | (colorFormat << 19) |
		 (outputFormat << 16) | (stride);
	...
	if (pDecInfo->interResChange == 1) {
		...
		VpuWriteReg(coreIdx, W4_SFB_OPTION,
			    (pDecInfo->openParam.fbc_mode << 20) |
				    (endian << 16) | (1 << 4) | (1 << 3) | 3);
		if (mapType == COMPRESSED_FRAME_MAP) {
		...
		if (mapType == LINEAR_FRAME_MAP &&
		    pDecInfo->openParam.cbcrOrder == CBCR_ORDER_REVERSED) {
		...
		APIDPRINT("REGISTER FB[%02d] Y(0x%08x), Cb(0x%08x) ", i, addrY,
			  addrCb);
		if (mapType == COMPRESSED_FRAME_MAP) {
			VpuWriteReg(coreIdx, W4_ADDR_FBC_Y_OFFSET0 + (i << 4),
				    pDecInfo->vbFbcYTbl[idx].phys_addr); /* Luma
									    FBC
									    offset
									    table
									  */
			VpuWriteReg(
				...
				pDecInfo->vbFbcCTbl[idx].phys_addr); /* Chroma
									    FBC
									    offset
									    table
									  */
			...
			VpuWriteReg(coreIdx, W4_ADDR_CR_BASE0 + (i << 4),
				    addrCr);
			VpuWriteReg(coreIdx, W4_ADDR_FBC_C_OFFSET0 + (i << 4),
				    0);
		...
		idx = 0;
		for (j = 0; j < q; j++) {
			regVal = (pDecInfo->openParam.fbc_mode << 20) |
				 (endian << 16) | (j == q - 1) << 4 |
				 ((j == 0) << 3);
			...
			VpuWriteReg(coreIdx, W4_SET_FB_NUM,
				    (startNo << 8) | endNo);

			for (i = 0; i < 8 && i < remain; i++) {
				if (mapType == LINEAR_FRAME_MAP &&
				    pDecInfo->openParam.cbcrOrder ==
					    CBCR_ORDER_REVERSED) {
				...
				VpuWriteReg(coreIdx,
					    W4_ADDR_LUMA_BASE0 + (i << 4),
					    addrY);
				VpuWriteReg(coreIdx,
					    W4_ADDR_CB_BASE0 + (i << 4),
					    addrCb);
				APIDPRINT(
					"REGISTER FB[%02d] Y(0x%08x), Cb(0x%08x) ",
					i, addrY, addrCb);
				if (mapType == COMPRESSED_FRAME_MAP) {
					VpuWriteReg(coreIdx,
						    W4_ADDR_FBC_Y_OFFSET0 +
							    (i << 4),
						    pDecInfo->vbFbcYTbl[idx]
							    .phys_addr); /* Luma
									    FBC
									    offset
									    table
					...
					VpuWriteReg(
						coreIdx,
						W4_ADDR_FBC_C_OFFSET0 +
							(i << 4),
						pDecInfo->vbFbcCTbl[idx]
							.phys_addr); /* Chroma
									    FBC
									    offset
									    table
			...
			VpuWriteReg(coreIdx, W4_ADDR_WORK_BASE,
				    pDecInfo->vbWork.phys_addr);
			VpuWriteReg(coreIdx, W4_WORK_SIZE,
				    pDecInfo->vbWork.size);
			VpuWriteReg(coreIdx, W4_WORK_PARAM, 0);

			// CODA7Q needs temporal buffer address.
			VpuWriteReg(coreIdx, W4_ADDR_TEMP_BASE,
				    pDecInfo->vbTemp.phys_addr);
			VpuWriteReg(coreIdx, W4_TEMP_SIZE,
				    pDecInfo->vbTemp.size);
	...
	if (inst->productId == PRODUCT_ID_7Q) {
		if (ConfigSecAXICoda7(coreIdx,
				      pDecInfo->openParam.bitstreamFormat,
				      &pDecInfo->secAxiInfo,
				      pDecInfo->initialInfo.picWidth,
				      pDecInfo->initialInfo.picHeight,
				      sequenceInfo->profile) == 0) {
			return RETCODE_INSUFFICIENT_RESOURCE;
		}
	} else
#endif
...
RetCode Wave4VpuDecFlush(CodecInst *instance,
			 FramebufferIndex *framebufferIndexes, Uint32 size)
{
	...
	if (framebufferIndexes != NULL) {
		for (j = 0, i = W4_RET_DEC_DISPLAY_INDEX_0;
		     i <= W4_RET_DEC_DISPLAY_INDEX_15 && j < size;
		     i += 4, j++) {
...
RetCode Wave4VpuBuildUpEncParam(CodecInst *instance, EncOpenParam *param)
{
	...
	if (instance->productId == PRODUCT_ID_7Q) {
		...
		else if (param->bitstreamFormat == STD_MPEG4 ||
	...
	CVI_VC_MEM("vbWork, phys_addr = 0x%llX, size = 0x%X\n",
		   pEncInfo->vbWork.phys_addr, pEncInfo->vbWork.size);

	sprintf(ionName, "VENC_%d_H265_WorkBuffer", instance->s32ChnNum);
	if (VDI_ALLOCATE_MEMORY(instance->coreIdx, &pEncInfo->vbWork, 0,
				ionName) < 0) {
	...
	vdi_clear_memory(instance->coreIdx, pEncInfo->vbWork.phys_addr,
