...
int main(int argc, char **argv) {
  ...
  printf("create alpha,width:%d,height:%d\n", width, height);
  // create horizontal gradient alpha image
  IVE_DST_IMAGE_S alpha;
  CVI_IVE_CreateImage(handle, &alpha, IVE_IMAGE_TYPE_U8C1, width, height);
  ...
  CVI_S32 ret = CVI_IVE_Image2VideoFrameInfo(&src1_yuv, &vf_src1);
  ret = CVI_IVE_Image2VideoFrameInfo(&src2_yuv, &vf_src2);
  ret = CVI_IVE_Image2VideoFrameInfo(&alpha, &vf_alpha);

  uint8_t *p_src2_y = (uint8_t *)malloc(src2_yuv.u16Stride[0] * height);
  memcpy(p_src2_y, src2_yuv.pu8VirAddr[0], src2_yuv.u16Stride[0] * height);

  struct timeval t0, t1;
  gettimeofday(&t0, NULL);
  ret = CVI_IVE_Blend_Pixel_Y(handle, &vf_src1, &vf_src2, &vf_alpha);
  ...
  if (ret == CVI_SUCCESS) {
    ...
    if (CVI_SUCCESS == cpu_ref(width, height, src1.u16Stride[0], src1_yuv.pu8VirAddr[0], p_src2_y,
                               alpha.pu8VirAddr[0], src2_yuv.pu8VirAddr[0])) {
      printf("check ok\n");
    }
  }
...
int cpu_ref(int width, int height, int stride, uint8_t *src_img1, uint8_t *src_img2,
  ...
  for (size_t r = 0; r < height; r++) {
    CVI_U8 *src1 = src_img1 + r * stride;
    CVI_U8 *src2 = src_img2 + r * stride;
    CVI_U8 *alpha1 = alpha_img + r * stride;
    CVI_U8 *dst = dst_img + r * stride;

    for (size_t i = 0; i < width; i++) {
      int src_val = src1[i] * alpha1[i] + src2[i] * (255 - alpha1[i]);
      int retval = src_val / 256.0 + 0.5;
      if (retval > 255) retval = 255;
      if (retval != dst[i]) {
        printf("fail,[%zu] u8_a:%d,u8_b:%d,u8_wa:%u,TPU %d, CPU %d,srcval:%d\n", i, src1[i],
               src2[i], alpha1[i], dst[i], retval, src_val);
        //   printBits(2, &src_val);
        printf("\n");
        return CVI_FAILURE;
      }
