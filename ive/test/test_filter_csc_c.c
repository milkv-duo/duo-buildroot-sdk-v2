...
int main(int argc, char **argv) {
  ...
  IVE_DST_IMAGE_S srcBuf;
  CVI_IVE_CreateImage(handle, &srcBuf, IVE_IMAGE_TYPE_U8C3_PLANAR, width, height);
  IVE_DST_IMAGE_S dst;
  CVI_IVE_CreateImage(handle, &dst, IVE_IMAGE_TYPE_U8C3_PLANAR, width, height);

  printf("Run TPU Filter + CSC.\n");
  CVI_S8 arr[] = {1,  4,  7, 4, 1,  4,  16, 26, 16, 4, 7, 26, 26,
                  41, 26, 7, 4, 16, 26, 16, 4,  1,  4, 7, 4,  1};

  IVE_FILTER_AND_CSC_CTRL_S iveFltCtrl;
  memcpy(iveFltCtrl.as8Mask, arr, 25 * sizeof(CVI_S8));
  iveFltCtrl.enMode = IVE_CSC_MODE_PIC_RGB2GRAY;
  iveFltCtrl.u16Norm = 273;

  struct timeval t0, t1;
  gettimeofday(&t0, NULL);
  for (size_t i = 0; i < total_run; i++) {
    CVI_IVE_FilterAndCSC(handle, &src, &srcBuf, &dst, &iveFltCtrl, 0);
  }
  ...
  if (total_run == 1) {
    ...
    CVI_IVE_WriteImage(handle, "test_filter_csc_c.png", &dst);
  } else {
