...
int main(int argc, char** argv) {
  ...
  IVE_DST_IMAGE_S dst;
  int width_dst = src.u16Width * 2;
  int height_dst = src.u16Height * 2;
  CVI_IVE_CreateImage(handle, &dst, IVE_IMAGE_TYPE_U8C1, width_dst, height_dst);
  printf("Image size destination is %d X %d, channel %d\n", dst.u16Width, dst.u16Height, nChannels);

  printf("Run CPU Image Resize.\n");

  IVE_RESIZE_CTRL_S ctrl;  // Currently a dummy variable
  ctrl.enMode = IVE_RESIZE_MODE_LINEAR;

  struct timeval t0, t1;
  gettimeofday(&t0, NULL);
  for (size_t i = 0; i < total_run; i++) {
    CVI_IVE_Resize(handle, &src, &dst, &ctrl, 0);
  }
  ...
  if (total_run == 1) {
    ...
    CVI_IVE_WriteImage(handle, "test_resize_c.png", &dst);
  }
