...
void readYUV420SP(IVE_HANDLE handle, const char *filename, const uint16_t width,
                  const uint16_t height, IVE_DST_IMAGE_S *img) {
  FILE *yuv = fopen(filename, "rb");
  uint8_t *ptr = (uint8_t *)malloc(width * height * 1.5 * sizeof(uint8_t));
  fread(ptr, width * height * 1.5, sizeof(uint8_t), yuv);
  fclose(yuv);
  CVI_IVE_CreateImage(handle, img, IVE_IMAGE_TYPE_YUV420SP, width, height);
  uint8_t *ptr2 = ptr;
  ...
  for (int i = 0; i < height_2; i++) {
    memcpy(img->pu8VirAddr[1] + i * img->u16Stride[1], ptr2, width * sizeof(uint8_t));
    ptr2 += width;
  }
  free(ptr);
...
int main(int argc, char **argv) {
  ...
  IVE_SRC_IMAGE_S src;
  readYUV420SP(handle, file_name, width, height, &src);

  IVE_DST_IMAGE_S dst;
  CVI_IVE_CreateImage(handle, &dst, IVE_IMAGE_TYPE_YUV420SP, width, height);
  ...
  CVI_IVE_CreateImage(handle, &dst2, IVE_IMAGE_TYPE_YUV420SP, width * 2, height * 2);
  ...
  CVI_IVE_CreateImage(handle, &dst3, IVE_IMAGE_TYPE_YUV420SP, width, height);
  ...
  writeYUV420SP(handle, "result.yuv", width, height, &dst);
  writeYUV420SP(handle, "result_big.yuv", width * 2, height * 2, &dst2);
  writeYUV420SP(handle, "result_rearrange.yuv", width, height, &dst3);
