...
static CVI_S32 InitEnc(void)
{
    ...
    VENC_CHN VencChn[] = {0};
    PAYLOAD_TYPE_E enPayLoad = codec == "h264"? PT_H264: PT_H265;
    ...
    _initInputCfg(&pIc);
    strcpy(pIc.codec, codec == "h264"? "264": "265");
    pIc.rcMode = 0; // cbr
    pIc.iqp = 38;
    pIc.pqp = 38;
    pIc.gop = 60;
    pIc.bitrate = 1800; // if fps = 20
    pIc.firstFrmstartQp = 51;
    pIc.num_frames = -1;
    pIc.framerate = 25;
    pIc.maxQp = 42;
    pIc.minQp = 26;
...
void *audioInput(void *arg)
{
    ...
    while (gRun) {
        ...
        CVI_RTSP_WriteFrame(gCTX, session->audio, &data);
    }
...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 && 0 == strcmp(argv[1], "h265")) {
        codec ="h265";
    }
    ...
    attr.video.codec = codec == "h264"? RTSP_VIDEO_H264: RTSP_VIDEO_H265;
    attr.audio.codec = RTSP_AUDIO_PCM_L16;
    attr.audio.sampleRate = sampleRate;

    snprintf(attr.name, sizeof(attr.name), "%s", codec.c_str());
    ...
    pthread_create(&audioThread, NULL, audioInput, (void *)session);
