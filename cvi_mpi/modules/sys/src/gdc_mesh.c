...
static CVI_S32 _convert_1st_src_midxy(int num_tilex_s1, CVI_U32 mesh_1st_size,
	COORD2D_INT_HW *src_1st_list, int tidx, int tidy, CVI_U8 *ptr,
	CVI_U32 *offset_p)
{
	CVI_U32 offset = *offset_p;
	CVI_U32 idx;
...
{
	// packed data in dram for HW
	CVI_U8 *ptr = (CVI_U8 *)src_1st_list_1d;
	CVI_U32 offset = 0;
	CVI_S32 ret;
	...
	for (int tidy = 0; tidy < num_tiley_s1; tidy++) {
		for (int tidx = 0; tidx < num_tilex_s1; tidx++) {
			...
			// # 256 bytes per tile
			for (CVI_U32 i = 0; i < 13; i++)
				ptr[offset++] = 0xab;

		}
	}

	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  offset=%d, mesh_1st_size=%d\n",
	...
	if (pstLDCAttr->stGridInfoAttr.Enable) {
		...
		for (int knotidx = 0; knotidx < 4; knotidx++) {
			...
			if (use_count == 4) {
				...
				xcor = (CVI_U32)_double2Int_s13_10(map_src_knot.xcor);
			} else {
				xcor = 0;
			}
		...
		for (int knotidx = 0; knotidx < 4; knotidx++) {
			...
			if (swmesh_hit_index[knotidx] == 0xFFFFFFFF) {
				CVI_TRACE_SYS(CVI_DBG_ERR,
					...
					knotidx, tidx, tidy);
				CVI_TRACE_SYS(CVI_DBG_ERR, "    midx = %d, midy = %d,\n", midx,
					midy);
				CVI_TRACE_SYS(CVI_DBG_ERR,
					"    cur_dst_mesh[%d] = (%d, %d)\n", knotidx,
					cur_dst_mesh[knotidx].xcor,
					cur_dst_mesh[knotidx].ycor);
				CVI_TRACE_SYS(CVI_DBG_ERR, "    DEBUG STASRT!!\n");
				return CVI_ERR_SYS_ILLEGAL_PARAM;
			}
			...
			int xidx = tidx * MESH_NUM_ATILE + midx;
			int yidx = tidy * MESH_NUM_ATILE + midy;
			CVI_U32 xcor = (CVI_U32)_double2Int_s13_10(map_src_knot.xcor);
			CVI_U32 offset =
						   ...
						   int mesh_horcnt, int mesh_vercnt, const LDC_ATTR_S *pstLDCAttr)
{
	...
	for (int tidy = 0; tidy < num_tiley_s1; tidy++) {
		...
		CVI_TRACE_SYS(CVI_DBG_DEBUG, "    OFFLINE tidy = %d\n", tidy);
	}
...
static void _convert_2nd_src_midxy(int num_tilex_s2,
	COORD2D_INT_HW *src_2nd_list, int tidx, int tidy, CVI_U8 *ptr,
	CVI_U32 *offset_ptr)
{
	CVI_U32 offset = *offset_ptr;
	CVI_U32 idx;
...
{
	// packed data in dram for HW
	CVI_U8 *ptr = (CVI_U8 *)src_2nd_list_1d;
	CVI_U32 offset = 0;
	CVI_U32 map_file_size = num_tiley_s2 * num_tilex_s2 * 256;
...
static CVI_S32 _fill_src_2nd_list(int num_tilex_s2, LDC_ATTR *cfg,
	...
	CVI_U32 *swmesh_hit_index, int mesh_horcnt, int mesh_vercnt)
{
	CVI_U32 max_mesh_num = 9 * (mesh_vercnt * mesh_horcnt);
	...
	for (int knotidx = 0; knotidx < 4; knotidx++) {
		...
		if (swmesh_hit_index[knotidx] == 0xFFFFFFFF) {
			CVI_TRACE_SYS(CVI_DBG_ERR, "    !!!! 2ndSTAG----ERROR !!!\n");
			return CVI_ERR_SYS_ILLEGAL_PARAM;
		}
		...
		CVI_U32 xcor = (CVI_U32)_double2Int_s13_10(map_src_knot.xcor);
		CVI_U32 idx = (yidx * (num_tilex_s2 * MESH_NUM_ATILE) + xidx) * 4 +
			       knotidx;
	...
	CVI_U32 mesh_2nd_size, int mesh_horcnt, int mesh_vercnt)
{
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    rotate mesh table by 90 degree done!!\n");
	...
	for (int tidy = 0; tidy < num_tiley_s2; tidy++) {
		...
		CVI_TRACE_SYS(CVI_DBG_DEBUG, "    2nd Stage OFFLINE tidy = %d\n", tidy);
	}
					 ...
					 int view_h, int mesh_horcnt, int mesh_vercnt, int rgn_idx)
{
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    view_h = %d,\n", view_h);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    view_w = %d,\n", view_w);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    mesh_horcnt = %d,\n", mesh_horcnt);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    mesh_vercnt = %d,\n", mesh_vercnt);

	int mesh_xstep = ((1 << PHASEBIT) * (view_w) / mesh_horcnt); //.2
	int mesh_ystep = ((1 << PHASEBIT) * (view_h) / mesh_vercnt); //.2

	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    mesh_xstep = %d,\n", mesh_xstep);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    mesh_ystep = %d,\n", mesh_ystep);
	...
	for (int j = 0; j < mesh_vercnt; j++) {
		...
		if (j &&
			...
			CVI_TRACE_SYS(CVI_DBG_WARN, "    Check The Position: ");
			CVI_TRACE_SYS(CVI_DBG_WARN, "      [%d] DstRgnMeshInfo[%d * %d].knot[0].ycor = %10f\n",
					rgn_idx, j, mesh_horcnt,
					rgn_attr[rgn_idx].DstRgnMeshInfo[j * mesh_horcnt].knot[0].ycor);
			CVI_TRACE_SYS(CVI_DBG_WARN, "      [%d] DstRgnMeshInfo[(%d - 1) * %d].knot[3] = %10f\n",
					rgn_idx, j, mesh_horcnt,
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "    PASS2!!!!\n\r");
	return CVI_SUCCESS;
}
...
void mesh_gen_rotation(SIZE_S in_size, SIZE_S out_size, ROTATION_E rot,
		       CVI_U64 mesh_phy_addr, void *mesh_vir_addr)
{
	(void)in_size;
	(void)out_size;
	(void)rot;
...
CVI_S32 mesh_gen_ldc(SIZE_S in_size, SIZE_S out_size,
	const LDC_ATTR_S *pstLDCAttr, CVI_U64 mesh_phy_addr,
	void *mesh_vir_addr, ROTATION_E rot)
{
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  1st src_1st_list=%p(%d), src_2nd_list=%p(%d)\n",
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "mesh_horcnt, mesh_vercnt (%d, %d\n)", mesh_horcnt, mesh_vercnt);
	...
	if (!src_1st_list || !src_2nd_list || !cfg || !rgn_attr || !cfg->DstRgnMeshInfo ||
		...
		CVI_TRACE_SYS(CVI_DBG_ERR, "  fail to alloc mesh\n");
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  ori_src_width = %d,\n", ori_src_width);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  ori_src_height = %d,\n", ori_src_height);

	// In LDC Processing, width & height  aligned to TILESIZE **
	int src_width_s1 = ((ori_src_width + TILESIZE - 1) / TILESIZE) * TILESIZE;
	int src_height_s1 = ((ori_src_height + TILESIZE - 1) / TILESIZE) * TILESIZE;

	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  src_width_s1 = %d,\n", src_width_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  src_height_s1 = %d,\n", src_height_s1);
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.stage2_rotate_type = %d,\n", reg.stage2_rotate_type);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.bg_color_y_r  = %d,\n", reg.bg_color_y_r);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.bg_color_u_g  = %d,\n", reg.bg_color_u_g);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.bg_color_v_b  = %d,\n", reg.bg_color_v_b);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.src_xstart_s1 = %d,\n", reg.src_xstr_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.src_xend_s1   = %d,\n", reg.src_xend_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.src_xstart_s2 = %d,\n", reg.src_xstr_s2);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  reg.src_xend_s2   = %d,\n", reg.src_xend_s2);
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "cfg size %d\n", (int)sizeof(LDC_ATTR));
	...
	for (int rgn_idx = 0; rgn_idx < cfg->RgnNum; rgn_idx++) {
		...
		CVI_TRACE_SYS(CVI_DBG_ERR, "    _get_region_dst_mesh_list: %ldms\n",
...
#ifdef ENABLE_PROFILE
		clock_gettime(CLOCK_MONOTONIC, &end);
		CVI_TRACE_SYS(CVI_DBG_ERR, "    _get_region_src_mesh_list: %ldms\n",
				get_diff_in_us(start, end) / 1000);
#endif

		// debug msg
		CVI_TRACE_SYS(CVI_DBG_DEBUG, "  REGION %d done.\n", rgn_idx);
	}
...
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_SYS(CVI_DBG_ERR, "    _get_frame_mesh_list: %ldms\n",
			get_diff_in_us(start, end) / 1000);
#endif
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  dst_height_s1 = %d,\n", dst_height_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  dst_width_s1  = %d,\n", dst_width_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  src_height_s2 = %d,\n", src_height_s2);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  src_width_s2  = %d,\n", src_width_s2);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  dst_height_s2 = %d,\n", dst_height_s2);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  dst_width_s2  = %d,\n", dst_width_s2);

	// 1st-stage, in(1984, 1088 ) -> out(1984, 1088)
	// 2nd-stage, in(1088, 1984 ) -> out(1088, 1984)

	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  1st-stage, in(%d, %d ) -> out(%d, %d)\n",
		       src_width_s1, src_height_s1, dst_width_s1, dst_height_s1);
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  2nd-stage, in(%d, %d ) -> out(%d, %d)\n",
	...
	CVI_TRACE_SYS(CVI_DBG_DEBUG, "  tileNum: 1st stage =( %d X %d )   2nd stage = ( %d X %d )\n",
...
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_SYS(CVI_DBG_ERR, "    _offline_get_1st_src_mesh_table: %ldms\n",
			get_diff_in_us(start, end) / 1000);
#endif
...
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_SYS(CVI_DBG_ERR, "    _offline_get_2nd_src_mesh_table: %ldms\n",
			get_diff_in_us(start, end) / 1000);
#endif
...
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_SYS(CVI_DBG_ERR, "    convert_1st_src_mesh_table: %ldms\n",
			get_diff_in_us(start, end) / 1000);
#endif
...
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_SYS(CVI_DBG_ERR, "    convert_2nd_src_mesh_table: %ldms\n",
			get_diff_in_us(start, end) / 1000);
#endif
