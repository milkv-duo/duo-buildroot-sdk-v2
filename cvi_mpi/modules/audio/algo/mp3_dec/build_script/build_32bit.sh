...
mkdir build
cd build
$DIR/../libmad32bit/configure --prefix=$DIR/../libmad32bit/install --enable-static --enable-shared --host=arm-linux-gnueabihf --with-gnu-ld=arm-linux-gnueabihf-ld  --build=arm --enable-fpm=arm CFLAGS=$CFLAGS

echo "start make------------------------------------"
make clean
make -j $NPROC
make install
cp -af $DIR/../libmad32bit/install/* $DIR/../libmad_output/32bit/
cd ..
rm -rf build
