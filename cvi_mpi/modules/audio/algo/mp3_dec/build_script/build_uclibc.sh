...
mkdir build
cd build
$DIR/../libmaduclibc/configure --prefix=$DIR/../libmaduclibc/install --enable-static --enable-shared --host=arm-cvitek-linux-uclibcgnueabihf --with-gnu-ld=arm-cvitek-linux-uclibcgnueabihf-ld --build=arm

echo "start make------------------------------------"
make clean
make -j $NPROC
make install
cp -af $DIR/../libmaduclibc/install/* $DIR/../libmad_output/uclibc/
cd ..
rm -rf build
