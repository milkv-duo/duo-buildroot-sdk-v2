...
export CROSS_COMPILE_PATH_UCLIBC="$TOOLCHAIN_PATH"/gcc/riscv64-linux-musl-x86_64/bin/riscv64-unknown-linux-musl-
...
mkdir build
cd build
../../lame-3.100/configure --prefix=$DIR/../lame-3.100/cvi_output_musl64 --enable-static --enable-shared --host=riscv64-unknown-linux
...
make -j $NPROC
make install
cp -af ../../lame-3.100/cvi_output_musl64/* ../../lame_output/musl64/
cd ..
rm -rf build
